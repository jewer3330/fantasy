...
public class ABsManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            Util.Log("total ab: " + loadedAssetBundles.Count + " resloaded = " + Common.assetResources.Count);
            foreach (KeyValuePair<string, LoadedAssetBundle> pair in loadedAssetBundles)
            {
                Util.Log(pair.Value.assetbundlename + "  " + pair.Value.m_ReferencedCount);
            }
        }
    }
#endif
    ...
    public static AssetBundle LoadInternal(string assetBundleName)
    {
        ...
        if (bs != null && bs.Length > 0)
        {
            ...
            {
                Util.LogError(e.ToString() + "  " + assetBundleName);
                return null;
            }
        ...
        {
            Util.LogError("Load AssetBundle:" + assetBundleName + " error");
            return null;
        }
