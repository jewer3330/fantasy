...
public class FileUtils
{
    ...
    public static string TestCDN()
    {
        ...
        {
            //Common.HideLoading();
            UIMessageBox.Show("网络断开" + "(error : 701)", () =>
            {
    ...
    {
        string urlForFile = string.Format("{0}/{1}/{2}/{3}", url, ResSetting.GetChannelRes(), Version.LoadVersion(), path)
    ...
    {
        string urlPath = string.Format("{0}/{1}/{2}/{3}", url, ResSetting.GetChannelRes(), Version.LoadVersion(), path)
            + "?time=" + System.DateTime.Now.Ticks + "&r=" + UnityEngine.Random.Range(int.MinValue, int.MaxValue);
