...
public class MarkTool
{
    ...
    public static string assetrecord_build = "assetrecord_build.dat";
    public static string recordPath = "record.txt";
    public static string record_diff = "record_diff.txt";
    public static string record_new = "record_new.txt";
    public static string record_del = "record_del.txt";
    public static string record_build = "record_build.txt";
    ...
    [MenuItem("Tools/打包/删除记录文件", false, 1)]
    ...
    public static void FitterRecord(List<string> files)
    {
        ...
        foreach (var k in Analysis.fileInfos)
        {
            ...
            //            if (lower.EndsWith("mat") && k.Value.usedCount > 1)
            //            {
            //                results.Add(k.Value);
            //            }
            if (lower.EndsWith("jpg") || lower.EndsWith("png") || lower.EndsWith("psd"))
            {
                results.Add(k.Value);
            }
            else
            {
                if (k.Value.fileName.EndsWith("prefab") || k.Value.fileName.Contains("Resources"))
                {
                    results.Add(k.Value);
                }
            }
    ...
    [MenuItem("Tools/打包/生成记录文件", false, 2)]
    ...
    public static void SaveRecordFile(List<RecordInfo> records)
    {
        ...
        string tempPath = "record.txt";
        WriteRecordFile(content.ToArray(), tempPath);
    }
    ...
    [MenuItem("Tools/读取场景信息")]

    public static string[] GetSceneConfig()
    {
        string content = FileUtilTool.ReadFile("scenes.txt");
    ...
    [MenuItem("Tools/打包/标签/全部打标签", false, 3)]
    ...
    [MenuItem("Tools/打包/清除标签/全部", false, 5)]
    ...
    public static List<string> AssetAddBundleName(List<string> listAsset)
    {
        ...
        {
            string r = string.Format("{0}\t{1}", asset.Replace("\\", "/"), GetAssetBundleName(asset));
            listRecord.Add(r);
        }
        return listRecord;
    }
