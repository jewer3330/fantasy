...
namespace NodeCanvas.Framework{
    ...
    public class Blackboard : MonoBehaviour, ISerializationCallbackReceiver, IBlackboard{
		...
		public string Save(string saveKey){
			...
			LuaUtil.PlayerPreSaveString(saveKey, json);
			return json;
		}
		...
		public bool Load(string saveKey){
			...
			var json = LuaUtil.PlayerPreGetString(saveKey);
...
//				Debug.Log("No data to load blackboard variables from key " + saveKey);
        ...
        public bool Deserialize(BlackboardSource bb)
        {
            if (bb == null)
            {
                return false;
            ...
            foreach (var pair in bb.variables)
            {
                ...
                {
                    _blackboard.SetValue(pair.Key, pair.Value.Clone());
                }
                else
                {
                    _blackboard.variables[pair.Key] = pair.Value.Clone() as Variable;
                }
            }
        ...
        public System.Xml.XmlDocument XmlSave()
        {
            return _blackboard.XmlSave();
        }
        public bool XmlLoad(System.Xml.XmlDocument xmlDoc)
        {
            return _blackboard.XmlLoad(xmlDoc);
        }
    }
}