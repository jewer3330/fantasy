	...
	abstract public class BTNode : Node {
		...
		protected override UnityEditor.GenericMenu OnContextMenu(UnityEditor.GenericMenu menu){
			menu.AddItem(new UnityEngine.GUIContent("Breakpoint"), isBreakpoint, ()=>{
                ...
                GraphOwner owner = graph.agent != null && graph.agent is GraphOwner ? (GraphOwner)graph.agent : null;
                if (owner == null)
                {
                    owner = graph.agent.gameObject.GetComponent<GraphOwner>();
                }
                if (owner != null)
                {
                    owner.OnSetBreakPoint(ID, isBreakpoint);
                }
            });
