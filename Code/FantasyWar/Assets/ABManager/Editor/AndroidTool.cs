...
public class AndroidTool
{
    ...
    public static string firstScene = "Assets/BattleCity/Scenes/Launcher.unity";
    ...
    public static void ShowResourcesFolder(bool b)
    {
        if (b)
        {
            ...
            AssetDatabase.MoveAsset("Assets/ResourceTemp", "Assets/Resources");
        }
        else
        {
            ...
            AssetDatabase.MoveAsset("Assets/Resources", "Assets/ResourceTemp");
        }
    }
    ...
    public static void BuildApk(Version v, string location)
    {
        ...
        FileUtilTool.CreateFolderForFile(location);
        string[] levels = new string[] { firstScene };
        try
        {
...
http://zjlmdl.joygame.cn/zjlm/close_beta", false);
    ...
    [MenuItem("Package/安卓/安卓完整包--不重新打资源--自动上传资源")]
    ...
    /// 这个接口外部脚本调用
    /// </summary>
    [MenuItem("Package/安卓/安卓完整包--不重新打资源--上传APK")]
    ...
    [MenuItem("Package/安卓/上传APK")]
    public static void UploadAPK()
    {
        try
        {
            string apkPath = string.Format("{0}/{1}/{2}", ResSetting.AssetBundlesOutputPath, ResSetting.GetChannelRes(), string.Format("{0:yyyyMMddHHmm}", System.DateTime.Now));
            ...
            string targetPath = string.Format("\\\\192.168.2.85\\FtpData\\{0}\\TankGirl_{1}_{2}({3}).res{4}_full.apk",
