...
namespace Assets.Editor.AnimatorEditor
{
    public class AETemplate
    {
        ...
        public void ParseFromLua(IntPtr L, LuaTable table)
        {
            ...
            {
                if (key == "Layers")
                    ParseLayers(L, value as LuaTable);
                else if (key == "CullingMode")
                    SetCullingMode(value.ToString());
                else if (key == "Parameters")
                    ParseParameters(L, value as LuaTable);
            });
        }

        private void ParseLayers(IntPtr L, LuaTable table)
        {
            LuaUtil.TravelTable(L, table, (key, value) =>
            {
                AELayer layer = new AELayer();
                layer.ParseFromLua(L, value as LuaTable);
                Layers.Add(layer);
            });
        }

        private void ParseParameters(IntPtr L, LuaTable table)
        {
            LuaUtil.TravelTable(L, table, (key, value) =>
            {
                AEParameter parameter = new AEParameter();
                parameter.ParseFromLua(L, value as LuaTable);
