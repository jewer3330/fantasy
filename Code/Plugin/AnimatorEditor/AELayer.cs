        ...
        public void ParseFromLua(IntPtr L, LuaTable table)
        {
            LuaUtil.TravelTable(L, table, (key, value) =>
            {
                ...
                if (key == "LayerName")
                    ...
                    ParseStates(L, value as LuaTable);
                else if (key == "BlendTrees")
                    ParseBlendTrees(L, value as LuaTable);
                else if (key == "Transitions")
                    ParseTransitions(L, value as LuaTable);
            });
        }

        private void ParseStates(IntPtr L, LuaTable table)
        {
            LuaUtil.TravelTable(L, table, (key, value) =>
            {
                AEState state = new AEState();
                state.ParseFromLua(L, value as LuaTable);
                States.Add(state);
            });
        }

        private void ParseBlendTrees(IntPtr L, LuaTable table)
        {
            LuaUtil.TravelTable(L, table, (key, value) =>
            {
                AEBlendTree tree = new AEBlendTree();
                tree.ParseFromLua(L, value as LuaTable);
                Trees.Add(tree);
            });
        }

        private void ParseTransitions(IntPtr L, LuaTable table)
        {
            LuaUtil.TravelTable(L, table, (key, value) =>
            {
                AETransition transition = new AETransition();
                transition.ParseFromLua(L, value as LuaTable);
