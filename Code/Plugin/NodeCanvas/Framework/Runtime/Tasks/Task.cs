...
namespace NodeCanvas.Framework{
	...
	abstract public partial class Task {
        ...
        virtual public void UpdateAgentAndBB()
        {
            if (!Set(ownerSystem.agent, ownerSystem.blackboard))
            {
                LogMgr.LogErrorFormat("BehaviorTree valid fail! action name: {0}", name);
            }
        }
