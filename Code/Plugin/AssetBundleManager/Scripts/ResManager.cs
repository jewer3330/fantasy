...
public class ResManager : MonoBehaviour
{
   ...
   /// <returns></returns>
    public static Object Load(string path, Object user, ObjectState.State state = ObjectState.State.Normal)
    {
        ...
        switch (Instance.pattern)
        {
            ...
            case Pattern.AssetBundle:
                ...
                if (user != null)
                    Common.AddABUser(user, path, state);
                break;
        }
        return obj;
    }
    ...
    public static string GetResourcesName(string path)
    {
        ...
        {
            string assetPath = string.Format("Assets/Resources/{0}", path);
            if (Table.IosLimit.Contains(assetPath))
            {
                path = Table.IosLimit.Get(assetPath).Path2;
    ...
    {
        string assetBundleName = string.Format("assets/resources/{0}.dat", path.ToLower());
        return assetBundleName;
    }

    public static string GetAssetName(string path)
    {
        string assetName = string.Format("assets/resources/{0}", path.ToLower());
        return assetName;
    }
