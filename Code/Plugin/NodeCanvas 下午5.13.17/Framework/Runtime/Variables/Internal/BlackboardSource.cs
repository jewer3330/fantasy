...
//			Debug.LogError(string.Format("<b>Blackboard:</b> Can't cast value '{0}' to blackboard variable of name '{1}' and type '{2}'", value != null? value.ToString() : "null", varName, variables[varName].varType.Name ));
        ...
        {
            return JSONSerializer.Serialize(typeof(BlackboardSource), this, false);
        }
        ...
        {
            var bb = JSONSerializer.Deserialize<BlackboardSource>(json);
            if (bb == null)
            {
                return false;
            }
            ...
            InitializePropertiesBinding(propertiesBindTarget, true);
            return true;
        }
        public XmlDocument XmlSave()
        {
            XmlDocument doc = new XmlDocument();
            XmlElement root = doc.CreateElement("blackboard");
            root.SetAttribute("version", Convert.ToString(1.0f));
            root.SetAttribute("name", name);
            doc.AppendChild(root);
            var enumVar = _variables.GetEnumerator();
            while (enumVar.MoveNext())
            {
                XmlElement varElement = doc.CreateElement("Variable");
                if(enumVar.Current.Value.ToXmlElement(varElement))
                {
        ...
        public bool XmlLoad(XmlDocument xmlDoc)
        {
            return true;
        }
    }
